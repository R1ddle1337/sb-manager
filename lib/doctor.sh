...
status_summary() {
  local sb_ver cf_ver nodes enabled certs mode service_state
...
doctor_repair_runtime() {
  require_root
  local candidate
  log_info '修复核心链接、路径权限、配置权限与 systemd 运行状态…'
  ...
  if [[ "$SBM_SKIP_SYSTEMD" != 1 ]]; then
    service_exists "$SBM_SERVICE" || die "缺少 $SBM_SERVICE，请重新执行安装命令。"
...
doctor_run() {
  local repair=${1:-0} failures=0 warnings=0 tmp node id protocol port kind domain path end epoch days
  local enabled endpoint core_target

  if [[ "$repair" == 1 ]]; then
    # Run the repair in a subshell so a defensive `die` does not prevent the
    # diagnostic report from explaining any remaining problems.
    if ! (doctor_repair_runtime); then
      log_error '自动修复未完成，继续输出诊断结果。'
    fi
  fi

  printf '%s\n' '---- sb doctor ----'
  if jq -e . "$SBM_STATE" >/dev/null 2>&1; then
    check_line PASS '状态文件 JSON 有效'
  else
    check_line FAIL '状态文件损坏'
    failures=$((failures + 1))
  fi
  ...
  if [[ -x "$SBM_SING_BOX_BIN" ]]; then
    ...
    if [[ -n "$sb_version" ]]; then
      check_line PASS "sing-box $sb_version"
      core_usable=1
    else
      check_line FAIL 'sing-box 文件可执行，但无法读取版本信息'
      failures=$((failures + 1))
    fi
  else
  ...
  if id "$SBM_SERVICE_USER" >/dev/null 2>&1; then
    if service_user_can_execute_core; then
      check_line PASS "$SBM_SERVICE_USER 可执行 sing-box 核心"
    else
      check_line FAIL "$SBM_SERVICE_USER 无法执行 sing-box；通常是核心目录缺少遍历权限"
      failures=$((failures + 1))
    fi
    if service_user_can_read_config; then
      check_line PASS "$SBM_SERVICE_USER 可读取生成配置"
    else
      check_line FAIL "$SBM_SERVICE_USER 无法读取 $SBM_CONFIG"
      failures=$((failures + 1))
    fi
  else
  ...
  if [[ -x "$SBM_CLOUDFLARED_BIN" ]]; then
    ...
    if [[ -n "$cf_version" ]]; then
      check_line PASS "cloudflared $cf_version"
    else
      check_line WARN 'cloudflared 文件存在，但无法读取版本信息'
      warnings=$((warnings + 1))
    fi
  else
  ...
  if [[ "$SBM_SKIP_SYSTEMD" != 1 ]]; then
    if ! service_exists "$SBM_SERVICE"; then
      check_line FAIL "$SBM_SERVICE 单元缺失；请重新运行安装命令"
      failures=$((failures + 1))
    elif (( enabled == 0 )); then
      if service_active "$SBM_SERVICE"; then
        check_line WARN "$SBM_SERVICE 在无启用节点时仍运行；执行 sb doctor --repair 可切换到待机"
        warnings=$((warnings + 1))
      elif service_enabled "$SBM_SERVICE"; then
        check_line WARN "$SBM_SERVICE 当前虽已停止，但仍设置为开机启动；执行 sb repair 可彻底切换到待机"
        warnings=$((warnings + 1))
      else
        check_line PASS '暂无启用节点；sb-sing-box.service 已禁用并保持停止待机'
      fi
    elif service_active "$SBM_SERVICE"; then
      check_line PASS "$SBM_SERVICE 正在运行"
    else
      local active_state sub_state result exec_status
      active_state=$(systemctl show "$SBM_SERVICE" -p ActiveState --value 2>/dev/null || echo unknown)
      sub_state=$(systemctl show "$SBM_SERVICE" -p SubState --value 2>/dev/null || echo unknown)
      result=$(systemctl show "$SBM_SERVICE" -p Result --value 2>/dev/null || echo unknown)
      exec_status=$(systemctl show "$SBM_SERVICE" -p ExecMainStatus --value 2>/dev/null || echo unknown)
      check_line FAIL "$SBM_SERVICE 未运行（$active_state/$sub_state，Result=$result，ExecMainStatus=$exec_status）"
      failures=$((failures + 1))
  ...
  while IFS= read -r node; do
    ...
    while IFS= read -r kind; do
      if host_port_in_use "$kind" "$port"; then
        check_line PASS "$id 监听 ${port}/${kind^^}"
      else
        check_line FAIL "$id 未监听 ${port}/${kind^^}"
        failures=$((failures + 1))
      fi
    ...
    if [[ "$protocol" == anytls || "$protocol" == hysteria2 ]]; then
      ...
      if [[ -s "$path" ]]; then
        ...
        if (( days < 7 )); then
          check_line FAIL "$domain 证书仅剩 $days 天"
          failures=$((failures + 1))
        elif (( days < 20 )); then
          check_line WARN "$domain 证书剩余 $days 天"
          warnings=$((warnings + 1))
        else
          check_line PASS "$domain 证书剩余 $days 天"
        fi
      else
    ...
    if [[ "$protocol" != vmess-ws-cf && -z $(jq -r '.server_address // ""' <<<"$node") ]]; then
      ...
      if [[ "$protocol" == vmess-ws-cf ]]; then
        endpoint=$(jq -r '.domain // ""' <<<"$node")
      else
        endpoint=$(jq -r '.server_address // ""' <<<"$node")
      fi
      if [[ -n "$endpoint" && "$endpoint" =~ [A-Za-z] ]]; then
        if getent ahosts "$endpoint" >/dev/null 2>&1; then
          check_line PASS "$id 的地址可解析：$endpoint"
        else
          check_line WARN "$id 的地址当前无法解析：$endpoint"
          warnings=$((warnings + 1))
        fi
      fi
    fi
  ...
  if command_exists timedatectl; then
    if timedatectl show -p NTPSynchronized --value 2>/dev/null | grep -qx yes; then
      check_line PASS '系统时间已同步'
    else
      check_line WARN '无法确认系统时间已同步'
      warnings=$((warnings + 1))
    fi
  fi

  if [[ $(jq -r '.tunnel.mode' "$SBM_STATE") != none && "$SBM_SKIP_SYSTEMD" != 1 ]]; then
    if service_active "$SBM_TUNNEL_SERVICE"; then
      check_line PASS 'Cloudflare Tunnel 服务运行中'
    else
      check_line FAIL 'Cloudflare Tunnel 已配置但服务未运行'
      failures=$((failures + 1))
    fi
  fi
