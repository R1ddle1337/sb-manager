...
quick_refresh_disable() {
  if [[ "$SBM_SKIP_SYSTEMD" != "1" ]]; then systemctl disable --now "$SBM_QUICK_REFRESH_TIMER" >/dev/null 2>&1 || true; fi
...
write_tunnel_unit() {
  local mode=$1 node_id=$2 node port unit token_path
  node=$(require_vmess_node "$node_id"); port=$(jq -r '.port' <<<"$node"); unit="$SBM_SYSTEMD_DIR/$SBM_TUNNEL_SERVICE"
  mkdir -p "$SBM_SYSTEMD_DIR" "$SBM_VAR/cloudflared-home"
  if [[ "$mode" == fixed ]]; then
...
_tunnel_setup_fixed() {
  ...
  tmp=$(mktemp "$SBM_SECRETS/.cloudflared-token.XXXXXX"); printf '%s\n' "$token" >"$tmp"; chmod 0640 "$tmp"; set_group_if_exists "$SBM_SERVICE_USER" "$tmp"; mv "$tmp" "$SBM_TUNNEL_TOKEN_FILE"
  quick_refresh_disable
  _tunnel_update_state fixed "$id" "$domain" "$address"
  write_tunnel_unit fixed "$id"
  if [[ "$SBM_SKIP_SYSTEMD" != "1" ]]; then
    systemctl daemon-reload; systemctl enable "$SBM_TUNNEL_SERVICE" >/dev/null; systemctl restart "$SBM_TUNNEL_SERVICE"
    service_active "$SBM_TUNNEL_SERVICE" || { journalctl -u "$SBM_TUNNEL_SERVICE" -n 30 --no-pager >&2 || true; die "cloudflared 未能启动。"; }
  fi
...
_tunnel_setup_quick() {
  local id=$1 node domain='' i candidate
  node=$(require_vmess_node "$id")
  _tunnel_update_state quick "$id" '' ''
  write_tunnel_unit quick "$id"
  if [[ "$SBM_SKIP_SYSTEMD" != "1" ]]; then
    write_quick_refresh_units
    systemctl daemon-reload
    systemctl enable "$SBM_TUNNEL_SERVICE" "$SBM_QUICK_REFRESH_TIMER" >/dev/null
    systemctl restart "$SBM_TUNNEL_SERVICE"
    systemctl restart "$SBM_QUICK_REFRESH_TIMER"
    for ((i=0;i<30;i++)); do
      domain=$(journalctl -u "$SBM_TUNNEL_SERVICE" --since '-2 min' --no-pager 2>/dev/null | grep -Eo 'https://[a-zA-Z0-9-]+\.trycloudflare\.com' | tail -n1 | sed 's#https://##')
      [[ -n "$domain" ]] && break
      sleep 1
    done
    if [[ -n "$domain" ]]; then _tunnel_update_state quick "$id" "$domain" "$domain"; log_ok "Quick Tunnel：$domain"; else log_warn "尚未从日志中取得 trycloudflare.com 域名；运行 sb tunnel refresh。"; fi
  fi
}
...
_tunnel_refresh_quick() {
  ...
  domain=$(journalctl -u "$SBM_TUNNEL_SERVICE" --since '-24 hours' --no-pager 2>/dev/null | grep -Eo 'https://[a-zA-Z0-9-]+\.trycloudflare\.com' | tail -n1 | sed 's#https://##')
...
_tunnel_stop() {
  local candidate
  if [[ "$SBM_SKIP_SYSTEMD" != "1" ]]; then systemctl disable --now "$SBM_TUNNEL_SERVICE" "$SBM_QUICK_REFRESH_TIMER" >/dev/null 2>&1 || true; fi
  rm -f "$SBM_SYSTEMD_DIR/$SBM_TUNNEL_SERVICE" "$SBM_SYSTEMD_DIR/$SBM_QUICK_REFRESH_SERVICE" "$SBM_SYSTEMD_DIR/$SBM_QUICK_REFRESH_TIMER"
  [[ "$SBM_SKIP_SYSTEMD" == "1" ]] || systemctl daemon-reload
  candidate=$(state_candidate)
  jq '
    . as $root |
...
tunnel_reconcile() {
  ...
  systemctl daemon-reload
  if [[ "$start" == 1 ]]; then
    systemctl enable "$SBM_TUNNEL_SERVICE" >/dev/null
    systemctl restart "$SBM_TUNNEL_SERVICE"
    if [[ "$mode" == quick ]]; then systemctl enable "$SBM_QUICK_REFRESH_TIMER" >/dev/null; systemctl restart "$SBM_QUICK_REFRESH_TIMER"; fi
  fi
}
...
tunnel_status() {
  ...
  if [[ "$SBM_SKIP_SYSTEMD" != "1" ]] && service_exists "$SBM_TUNNEL_SERVICE"; then systemctl status "$SBM_TUNNEL_SERVICE" --no-pager -l || true; fi
}
