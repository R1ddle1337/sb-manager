...
Run installation tests only in a disposable VM or the scripts' isolated test modes.

## Coding Style & Naming Conventions

Use Bash with `set -Eeuo pipefail`, two-space indentation, quoted expansions, and `local` function variables. Prefer `snake_case` functions and variables; reserve `SBM_*` names for exported configuration paths and runtime settings. Keep protocol logic in `protocols/<name>.sh` and cross-protocol behavior in `lib/`. Use `jq --arg`/`--argjson` to construct JSON instead of string interpolation. Protect mutating operations with `with_lock` and preserve atomic candidate-check-install-rollback behavior.
